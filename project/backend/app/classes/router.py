...
async def create_class(title: str, description: str, class_id: int,
...
async def create_class(class_id: int, task_id: int, task_service: Annotated[TaskService, Depends(task_service)],
                       user: User = Depends(get_current_user)):
...
async def update_class(title: str, description: str, class_id: int, task_id: int,
                       task_service: Annotated[TaskService, Depends(task_service)],
                       user: User = Depends(get_current_user)):
    update_task_user = await task_service.update_task(class_id=class_id, title=title, description=description,
                                                      task_id=task_id)
...
async def delete_class(task_id: int, task_service: Annotated[TaskService, Depends(task_service)],
                       user: User = Depends(get_current_user)):
